...
class pca:
    ...
    def num_effective_dims(self, percvar):
        # Based on the percentage of variance to be explained, return the number of effective dimensions
        evals_sum = np.sum(self.evals)
        evals_cumsum = np.cumsum(self.evals)
        evals_perc = evals_cumsum / evals_sum * 100
        result = np.searchsorted(evals_perc, percvar) + 1
        return result
